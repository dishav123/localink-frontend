...
function MyAppointments() {
  ...
  const handlePayment = async (appt) => {
    try {
      const { data } = await axios.post(
        `${backendUrl}/user/initiate-khalti`,
        { appointmentId: appt._id, amount: appt.amount },
        { headers: { token } },
      );
  ...
  useEffect(() => {
    // Khalti redirects back with ?pidx=xxx in the URL
      ...
      <div className="flex flex-col gap-3">
        {appointments.slice(0, 10).map((appt, index) => {
          ...
          return (
            ...
            >
              <div
                className={`absolute left-0 top-0 bottom-0 w-1 ${status.accent}`}
              />
              ...
              <div className="flex flex-col sm:flex-row gap-4 pl-5 pr-4 py-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-0.5">
                    ...
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${status.bg} ${status.textColor} ${status.border}`}
                    >
                      <span
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
                    <div>
                      <p className="text-neutral-400 uppercase tracking-wider text-[10px] font-semibold mb-0.5">
                        Date & Time
                      </p>
                    ...
                    <div>
                      ...
                      <p className="text-neutral-700">
                        {sp.yearsOfExperience} yrs · {sp.completedProjects}{" "}
                        projects
                      </p>
                    </div>
                    <div>
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 sm:min-w-[120px] border-t sm:border-t-0 border-neutral-100 pt-3 sm:pt-0">
                  ...
                  <div className="flex sm:flex-col gap-2">
                    {!appt.cancelled && !appt.isCompleted && !appt.payment && (
                      <>
                        {/* ── Pay Online button — triggers Khalti ── */}
                        <button
                          onClick={() => handlePayment(appt)}
                          className="text-xs px-4 py-2 rounded-full bg-[#5C2D91] text-white font-medium hover:bg-[#4a2475] transition-colors duration-150 shadow-sm hover:shadow flex items-center gap-1.5"
                        >
                          {/* Khalti purple to match their brand */}
                          <img
                            src="https://khalti.com/static/khalti-logo-white.svg"
                            alt="Khalti"
                    ...
                    {/* Already paid — show pay again option removed, show status */}
                    {!appt.cancelled && !appt.isCompleted && appt.payment && (
                      <span className="text-xs px-3 py-1.5 rounded-full bg-sky-50 text-sky-600 border border-sky-200 cursor-default">
                        ✓ Paid
                      </span>
                    )}
